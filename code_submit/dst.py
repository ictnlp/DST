...
class TransformerLanguageModelConfig(FairseqDataclass):
    ...
    low_bound: int = field(
        default=0,
        metadata={
            "help": "The low bound for Wait-k policy",
        },
    )
    top_bound: int = field(
        default=1024,
        metadata={
            "help": "The top bound for Wait-k policy",
        },
    )
    mass_ratio: float = field(
        default=0.5,
        metadata={
            "help": "The mass ratio for the main latency intervals:",
        },
    )
    ...
    def build_model(cls, args, task):
        ...
        if args.character_embeddings:
            ...
            embed_tokens = cls.build_embedding(
                args, task.target_dictionary, args.decoder_input_dim
            )

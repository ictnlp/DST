...
class LabelSmoothedCrossEntropyCriterionLMSiMTConfig(FairseqDataclass):
    ...
    train_threshold: float = field(
        default=0.0,
        metadata={"help": "parameter for \theta"},
    )
    curr_steps: int = field(
        default=10000,
        metadata={"help": "parameter for curriculum steps"},
    )
...
@register_criterion(
    "dst_loss", dataclass=LabelSmoothedCrossEntropyCriterionLMSiMTConfig
)
class LabelSmoothedCrossEntropyCriterion(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True, update_num=0):
        ...
        if self.training:
            mask_ratio = self.train_threshold + (1.0 - self.train_threshold) * math.exp(-update_num / float(self.curr_steps))
        else:
            mask_ratio = self.train_threshold
        # forward
        net_output = model([src_tokens, sample['net_input']['prev_output_tokens']], language_model_task=False, mask_ratio=mask_ratio)
        # cross_entropy_loss
        loss, nll_loss = self.compute_loss(model, net_output, sample, reduce=reduce)
        # latency constraint
        latency_loss = self.latency_loss(model, net_output[1]['merge_src_weight'], src_tokens, sample['net_input']['prev_output_tokens'])
        # summation and average constraints
        limit_src_sum, limit_src_avg = self.sum_and_avg_loss(model, net_output[1]['merge_src_weight'], src_tokens, sample['net_input']['prev_output_tokens'], net_output[1]['merge_signal'])
    ...
    def latency_loss(self, model, merge_src_delta, src_tokens, tgt_tokens):
    ...
    def sum_and_avg_loss(self, model, attend_src_weights, src_tokens, tgt_tokens, signal):
        bsz=src_tokens.size(0)
        tgt_len=tgt_tokens.size(1)
        src_len=src_tokens.size(1)
